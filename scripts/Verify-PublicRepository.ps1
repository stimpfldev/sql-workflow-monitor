...
foreach ($pattern in $forbiddenPatterns) {
    ...
    foreach ($match in $matches) {
        Add-VerificationError (
            "Archivo no permitido en el repositorio: " +
...
foreach ($file in Get-ChildItem -Path $repositoryRoot -Recurse -File) {
    ...
    foreach ($marker in $sensitiveKeyMarkers) {
        if ($content.Contains($marker)) {
            Add-VerificationError (
                "Material criptográfico sensible detectado: " +
...
if ($appSettings.AllowedHosts -eq "*" -or
    ...
    Add-VerificationError (
        "AllowedHosts debe ser explícito en la configuración.")
}
...
$productVersion =
    [string]$buildProps.Project.PropertyGroup.Version
...
$solutionPath =
    Join-Path $repositoryRoot "SqlWorkflowMonitor.slnx"
...
if ($errors.Count -gt 0) {
    throw "La verificación falló:`n- $($errors -join "`n- ")"
}

Write-Host "Repositorio validado: configuración, versión, SQL y controles de publicación correctos."
